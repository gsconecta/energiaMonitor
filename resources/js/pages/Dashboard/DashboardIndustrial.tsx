...
import type { Props as DashboardProps } from './Index';

export default function DashboardIndustrial({
    metricas,
    datos_grafica,
    dispositivo,
}: Pick<DashboardProps, 'metricas' | 'datos_grafica' | 'dispositivo'>) {
    return (
        <div className="flex w-full flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                <div className="rounded-lg border bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    <p className="text-sm font-medium text-gray-500 dark:text-gray-400">
                        Potencia Actual (Global)
                    </p>
                    <p className="mt-2 text-2xl font-bold text-gray-900 dark:text-white">
                        {metricas?.potencia_actual_kw || 0} kW
                    </p>
                </div>
                <div className="rounded-lg border bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    <p className="text-sm font-medium text-gray-500 dark:text-gray-400">
                    ...
                    <p className="mt-2 text-2xl font-bold text-red-500">
                        {metricas?.potencia_maxima_kw || 0} kW
                    </p>
                </div>
                <div className="rounded-lg border bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    <p className="text-sm font-medium text-gray-500 dark:text-gray-400">
                    ...
                    <div className="mt-2 flex items-center gap-4">
                        {Array.from({
                            length: dispositivo?.num_fases || 1,
                        }).map((_, i) => {
                            const colorCanal =
                                (dispositivo?.[
                                    `color_canal_${i + 1}` as keyof typeof dispositivo
                                ] as string) || '#6366f1';
                            return (
                                <div
                                    key={i}
                                ...
                                >
                                    <span className="text-sm tracking-wider text-gray-500 uppercase dark:text-gray-400">
                                        L{i + 1} :
                                    </span>
                                    <span
                        ...
                        <div className="w-full rounded-lg border bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                            <h2 className="mb-4 text-lg font-semibold text-gray-900 dark:text-gray-100">
                                {dispositivo?.tiene_fotovoltaica
                                    ...
                                    : 'Evolución de Consumo de Potencia'}
                            </h2>
                        ...
                        <div className="w-full rounded-lg border bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                            ...
                            <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-3">
                                {Array.from({
                                    ...
                                    const v =
                                        (metricas?.[
                                            `voltaje_actual_${fase}` as keyof typeof metricas
                                        ] as number) ?? 0;
                                    const current =
                                        (metricas?.[
                                            `corriente_actual_${fase}` as keyof typeof metricas
                                        ] as number) ?? 0;
                                    const pf =
                                        (metricas?.[
                                            `factor_potencia_${fase}` as keyof typeof metricas
                                        ] as number) ?? 0;
                                    const q =
                                        (metricas?.[
                                            `q${fase}_var_actual` as keyof typeof metricas
                                        ] as number) ?? 0;

                                    const customColor = dispositivo?.[
                                        `color_canal_${fase}` as keyof typeof dispositivo
                                    ] as string | undefined;
                                    const defaultColors = [
                                        'text-amber-500',
                                        'text-purple-500',
                                    ...
                                    return (
                                        <div
                                            key={fase}
                                            className="flex flex-col rounded-lg border bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900"
                                        >
                                            ...
                                            <div className="grid grid-cols-3 gap-2 text-center">
                                                <div className="rounded bg-gray-50 p-2 dark:bg-gray-800/50">
                                                    <div className="mb-1 text-[10px] tracking-wider text-gray-500 uppercase dark:text-gray-400">
                                                        Voltaje
                                                    </div>
            ...
            <div className="mt-4 rounded-lg border bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <h3 className="mb-2 text-lg font-medium text-gray-900 dark:text-white">
                    Análisis por Zonas / Máquinas (Próximamente)
                </h3>
                <p className="text-sm text-gray-500 dark:text-gray-400">
                    Aquí se integrarán los consumos desglosados de cada Sitio en
                    esta Organización para detectar picos por fase y por
                    maquinaria.
                </p>
            </div>
        </div>
