...
class DashboardController extends Controller
{
    ...
    private function calcularMetricas($lecturas, $dispositivo, $ultimaLecturaReal = null)
    {
        ...
        $energiaRed = 0;
        $energiaFotovoltaica = 0;

        // Identificar qué canal corresponde a qué energía
        for ($i = 1; $i <= 3; $i++) {
            $tipo = $dispositivo->getTipoCanal($i);
            $energiaDelCanal = match ($i) {
                1 => $energiaCanal1,
                2 => $energiaCanal2,
                3 => $energiaCanal3,
                default => 0
            ...
            if ($tipo === 'red_electrica') {
                $energiaRed += $energiaDelCanal;
            } elseif ($tipo === 'fotovoltaica') {
                $energiaFotovoltaica += $energiaDelCanal;
            }
        }
        ...
        return [
            ...
            'voltaje_actual_1' => $ultimaLecturaReal?->voltaje_canal_1 ?? 0,
            'voltaje_actual_2' => $ultimaLecturaReal?->voltaje_canal_2 ?? 0,
            'voltaje_actual_3' => $ultimaLecturaReal?->voltaje_canal_3 ?? 0,
            'corriente_actual_1' => $ultimaLecturaReal?->corriente_canal_1 ?? 0,
            'corriente_actual_2' => $ultimaLecturaReal?->corriente_canal_2 ?? 0,
            'corriente_actual_3' => $ultimaLecturaReal?->corriente_canal_3 ?? 0,
            'factor_potencia_1' => $ultimaLecturaReal?->pf_canal_1 ?? 0,
            'factor_potencia_2' => $ultimaLecturaReal?->pf_canal_2 ?? 0,
            'factor_potencia_3' => $ultimaLecturaReal?->pf_canal_3 ?? 0,
            'q1_var_actual' => $ultimaLecturaReal?->calcularPotenciaReactivaCanal(1) ?? 0,
            'q2_var_actual' => $ultimaLecturaReal?->calcularPotenciaReactivaCanal(2) ?? 0,
            'q3_var_actual' => $ultimaLecturaReal?->calcularPotenciaReactivaCanal(3) ?? 0,
            'q_total_var_actual' => $ultimaLecturaReal?->calcularPotenciaReactivaTotal() ?? 0,
